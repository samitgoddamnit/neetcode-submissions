...
class Solution {
    ...
    public int diameterOfBinaryTree(TreeNode root) {
        //implement iterative dfs to find the diameter tomorrow.
        Stack<TreeNode> stack = new Stack();
        HashMap<TreeNode,Integer> cache = new HashMap();
        HashMap<TreeNode,Integer> diameters = new HashMap();
        int diameter = 0;
        cache.put(null,0);
        stack.push(root);
        while (!stack.empty()){
            ...
            if (!cache.containsKey(node.left)){
                ...
                int tmp_diameter = left_height + right_height;
                diameter = Math.max(diameter,tmp_diameter);
